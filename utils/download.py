...
from backend.src.models import Session, Point, Team, Tournament, Player, Role, GamePlayer, Game
from backend.src.models import Parameter, Stat, Arena
...
async def download_all():
    ...
    for i in await query_data(Arena):
        dict_temp = {'model': 'arena', 'fields': {}}
        dict_temp['fields']['id'] = i.id
        dict_temp['fields']['name_arena'] = i.name_arena
        dict_temp['fields']['id_point'] = i.id_point
        dict_temp['fields']['created_on'] = str(i.created_on)
        dict_temp['fields']['updated_on'] = str(i.updated_on)
        all_data.append(dict_temp)
    ...
    for i in await query_data(Tournament):
        ...
        dict_temp['fields']['id_arena'] = i.id_arena
        dict_temp['fields']['created_on'] = str(i.created_on)
        dict_temp['fields']['updated_on'] = str(i.updated_on)
        all_data.append(dict_temp)
    ...
    for i in await query_data(Game):
        dict_temp = {'model': 'game', 'fields': {}}
        dict_temp['fields']['id'] = i.id
    ...
    for i in await query_data(Stat):
        ...
        dict_temp['fields']['id_game'] = i.id_game
        dict_temp['fields']['created_on'] = str(i.created_on)
        dict_temp['fields']['updated_on'] = str(i.updated_on)
        all_data.append(dict_temp)

    for i in await query_data(GamePlayer):
        dict_temp = {'model': 'game_player', 'fields': {}}
        dict_temp['fields']['id'] = i.id
        dict_temp['fields']['id_game'] = i.id_game
        dict_temp['fields']['id_player'] = i.id_player
        dict_temp['fields']['created_on'] = str(i.created_on)
